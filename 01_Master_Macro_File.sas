...
List of macros contained in this file:
	...
	GetAuditFees3
		- Modifications to original macro
...
/*****************************************************************************************************************************/
/*  Program:        Merge Audit Analytics and Compustat                                           	 	 					 */
/*  Author:         Joost Impink                                                                                             */
/*  Last modified:  April 2016                                                                                               */
/*  Last Download:  April 2016                                                                                               */
/*****************************************************************************************************************************/


/* 

NOTE- I made 1 change: I altered the end of the code so that company_fkey is included in output of dataset

These changes are labeled as:  /*CHANGE FROM ORIGINAL CODE*/
...
data work.d_nomatch (drop = AUDITOR_FKEY AUDIT_FEES NON_AUDIT_FEES NAME_AA /*CHANGE FROM ORIGINAL CODE*/ COMPANY_FKEY);
set work.b_aa2;
if AUDIT_FEES eq .;
run;
...
proc sql;
    ...
    select a.*, b.AUDITOR_FKEY, b.COMPANY_FKEY /*CHANGE FROM ORIGINAL CODE*/, b.AUDIT_FEES, b.NON_AUDIT_FEES,
        b.MATCHQU_BALSH_ASSETS, b.MATCHQU_INCMST_REV_TTM, b.NAME as NAME_AA,
        b.BEST_EDGAR_TICKER
    from
        work.d_nomatch a,
...
proc sql;
    ...
    create table &dsout as
        ...
        select a.*, b.company_fkey /*CHANGE FROM ORIGINAL CODE*/, b.AUDITOR_FKEY, b.AUDIT_FEES, b.NON_AUDIT_FEES
        from
             &dsin a
        LEFT JOIN
            work.g_matched b
...
/*
    ...
    getAuditFees2 - Matching Compustat Funda with Audit fees (Audit Analytics)
	
	Modified version of Joost's original code. 5 main differences:

	1) Keeps all auditors associated with a given client-year (previous version kept only largest fees)
	2) Uses WRDS SEC Analytics suite to merge based on other CIK numbers associated w/ gvkey
	3) Allows user to specify which variables to pull from aa.auditfees
	4) Output datasets also contains company_fkey (to assist merging w/ other Audit Analytics datasets)
	5) Keeps observations with zero audit fees as zero (previous version set these as missing) 
 
    Macro variables
    ===============
        dsin        dataset in with variables: gvkey, fyear
        dsout       dataset out, with appended company_fkey, auditor_fkey and specified AA_Vars (missings for unmatched)
		AA_Vars		specify variables from aa.auditfees (ex: audit_fees, non_audit_fees, tax_fees... etc.)
    ...
    ============
        ...
        This macro uses the %runquit macro (this macro triggers an abort when an error occurs)
...
%macro getAuditFees3(dsin=, dsout=, AA_Vars= );
...
PROC SQL;
	create table work.a_spec1 as
	select a.*, b.conm, b.datadate, b.at, b.sale, b.tic
    from work.a_spec0 as a, comp.funda as b
...
proc sql;
	create table work.a_spec2 as
	select a.*, b.fic, b.sic, b.ADDZIP, b.cik
 	from work.a_spec1 as a left join comp.company as b
    on a.gvkey = b.gvkey;
%runquit;

/* Add second CIK from WRDS SEC Analytics Suite CIK-GVKEY Linktable */

proc sql;
	create table work.a_spec3 as
	select a.*, b.cik as CIK2 label 'CIK2'
	from work.a_spec2 as a left join wrdssec.wciklink_gvkey as b
	on a.gvkey = b.gvkey
	and a.cik ne b.cik
	and b.cik ne ""
	and b.datadate1 <= a.datadate <= b.datadate2 
/* Exclude "No Link" (Flag = 0) and "No Filings" (Flag = 1) */
	and b.flag >= 2;
%runquit;


data work.a_spec4 (drop = fic);
set work.a_spec3;
if FIC = 'USA';
zipcode_current = put(ADDZIP,$5.);
sic2 = substr(sic,1,2);
...
proc sql;
	create table work.b_aa1 as
    select a.*, b.name
    from aa.auditfees as a left join aa.Namesauditfees as b
...
proc sql;
	create table work.b_aa2 as
    select a.*, b.AUDITOR_FKEY, %do_over(values = &AA_Vars, between = comma, phrase = b.?), 
	b.COMPANY_FKEY, b.MATCHQU_BALSH_ASSETS, b.MATCHQU_INCMST_REV_TTM, b.name as NAME_AA, b.BEST_EDGAR_TICKER
    from work.a_spec4 as a left join work.b_aa1 as b
    on a.cik = b.COMPANY_FKEY
    and a.datadate - 15 <= b.FISCAL_YEAR_ENDED <= a.datadate + 15;
%runquit;
 
proc sql;
	create table work.b_aa3 as
    select a.*, b.auditor_fkey as auditor_fkey2,
	%do_over(values = &AA_Vars, between = comma, phrase = b.? as ?2) 
    from work.b_aa2 as a left join work.b_aa1 as b
    on a.cik2 = b.COMPANY_FKEY
	and a.cik ne b.COMPANY_FKEY
	and a.audit_fees = .
    and a.datadate - 15 <= b.FISCAL_YEAR_ENDED <= a.datadate + 15;
%runquit;
...
data work.b_aa4;
set  work.b_aa3;
%do_over(values = auditor_fkey &AA_Vars, between =, phrase = if ?2 ne . then ? = ?2;)
%do_over(values = auditor_fkey &AA_Vars, between =, phrase = drop ?2;) 
run;

/* If same auditor has multiple audit fees for same year, keep obersvation with largest fees */

proc sort data = work.b_aa4; by gvkey fyear auditor_fkey descending audit_fees; run;
proc sort data = work.b_aa4 nodupkey; by gvkey fyear auditor_fkey; run;

/* Separate Match sample and try to find match for remaining observations */
...
data work.d_nomatch (drop = AUDITOR_FKEY &AA_Vars NAME_AA MATCHQU_BALSH_ASSETS MATCHQU_INCMST_REV_TTM BEST_EDGAR_TICKER);
set work.b_aa4;
if AUDIT_FEES eq .;
run;
...
proc sql;
	create table work.d_nomatch2  as
    select a.*, b.AUDITOR_FKEY, %do_over(values = &AA_Vars, between = comma, phrase = b.?), 
    b.MATCHQU_BALSH_ASSETS, b.MATCHQU_INCMST_REV_TTM, b.NAME as NAME_AA, b.BEST_EDGAR_TICKER
    from work.d_nomatch as a, work.b_aa1 as b
...
proc sql;
	create table work.f_multiple3 as
    select paired, count(*) as numObs 
	from work.f_multiple2 
	group by paired;   
quit;
...
proc sql;
    ...
    select * 
    from work.f_multiple2 
    where paired in (select paired from work.f_multiple3 where numObs > 1); 
quit;
...
proc sort data = work.g_matched; by gvkey fyear auditor_fkey descending audit_fees; run;
proc sort data = work.g_matched nodupkey; by gvkey fyear auditor_fkey; run;

/*  Create dataset to be returned */
 
proc sql;
	create table &dsout as
	select a.*, b.company_fkey, b.auditor_fkey, %do_over(values = &AA_Vars, between = comma, phrase = b.?)
    from &dsin as a left join work.g_matched as b
	on a.gvkey = b.gvkey
    and a.fyear = b.fyear; 
quit;

/* Treat zero audit fees as missing */
/* 
data &dsout;
set &dsout;
if AUDIT_FEES eq 0 then AUDIT_FEES = .;
...
proc datasets library=work nolist;
   delete a_spec0 - a_spec4 b_aa1 - b_aa4 c_match d_nomatch d_nomatch2 
        e_single1 - e_single3 f_multiple1 - f_multiple4 g_matched;
%runquit;
