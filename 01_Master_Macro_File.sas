...
List of macros contained in this file:
	...
	GetAuditFees3
		- Merge Audit Analytics and Compustat
...
/****************************************************************************************************************************
	...
	Matching Compustat Funda with Audit fees (Audit Analytics)
		Author: Andrew Kitto (based on Joost Impink's original macro: http://www.wrds.us/index.php/repository/view/27)

	Description
	===========

	This macro identifies all audit fees associated with a list of firm years. If a company reports fees paid to multiple
	audit firms in a given firm year, the fees paid to each firm are listed (i.e. there can be more than 1 auditor_fkey

	Matching procedure
	==================

(1)	Match CIK (WRDS SEC Analytics Suite) to Company_Fkey (Audit Analytics)

	All valid Central Index Keys (CIKs) associated with a given firm year (gvkey-datadate) are gathered from the WRDS 
	SEC Analytics Suite. The initial match is on CIK - Company_Fkey, which is the unique identifier in Audit Analytics. 
	Most firm years have only 1 valid match (~95%).  However, for firm fyears with multiple CIK-Company fkey matches, 
	only 1 company_fkey is allowed to match to each gvkey-datadate firm year. The following variables from Audit Analytics 
	are used to prioritize matches:

		Assets: MATCHFY_BALSH_ASSETS (if missing, then MATCHQU_BALSH_ASSETS), compared to Compustat AT 
		Sales: MATCHFY_INCMST_REV_TTM (if missing, then MATCHQU_INCMST_REV_TTM), compared to Compustat Sale
		Ticker Symbol: BEST_EDGAR_TICKER, compared to Compustat Tic
		Firm Name: 'distance' of NAME (using spedis function), compared to Compustat Conm < 10

	'Tiebreaker' order is as follows: 1) Sales & Assets both within 1% 2) Ticker Match 3) Name Match 4) Difference in 
	Assets 5) Difference in Sales 6) total number of filings associated with CIK according to WRDS SEC Analytics Suite.

(2)	Match based on sales and assets

	For firm-years that do not have a cik-company_fkey match, a match is done on assets and sales. BOTH assets and sales 
	must to be within 1%. 

	For gvkey-company_fkey pairs that match sales & assets in multiple fiscal years, the pair is treated as a valid match 
	(no further restrictions on ticker, company name).

	For gvkey-company_fkey pairs that match sales & assets in only 1 year, either the the ticker symbol needs to be the 
	same, or the 'distance' (spedis) of the firmname needs to be lower than 10. In AA, firm name is in auditcblock 
	versus conm in Funda.

	Relevant variables in Auditfees:
		COMPANY_FKEY	
		FISCAL_YEAR_ENDED => match with compustat.datadate +- 7 days
		MATCHFY_BALSH_ASSETS, MATCHQU_BALSH_ASSETS (in millions) => total assets
		MATCHFY_INCMST_REV_TTM, MATCHQU_INCMST_REV_TTM => sales

	Potentially relevant variables (not used), could be used for matching on price on CRSP (not prcc_f in Funda, which is 
	based on datadate, not the actual end of year date):
		MATCHQU_PRICE_DATE
		MATCHQU_PRICE_CLOSE => match with compustat.prcc_f

	Macro variables
	===============
		dsin		dataset in with variables: gvkey, datadate
		dsout		dataset out, with appended: 
						-Compustat variable: fyear
						-Match Indicator (= 1 firm year has a valid match to audit analytics, 0 otherwise)
						-Audit Analytics merging variables: fiscal_year, fiscal_year_ended (to assist merging with other  
						 AA databases)
						-Any AA variables specified in input statatement (see below) 
		FeeVars		Specify any variables from the aa.auditfees dataset to be included in dsout
					(e.g. Audit_Fees, Non_Audit_Fees, Tax_Fees, etc.)
	Requirements
	============
		The macro assumes it has local access to:
		- comp.funda
		- wrdssec.wciklink_gvkey (WRDS SEC Analytics Suite)
		- aa.auditfees
		- aa.auditcblock 
	...
	Sample usage
	============
		data a_funda (keep = gvkey datadate);
		set comp.funda;
		if fyear >= 2010;
		if at > 0;
		...
		%getAuditFees2(dsin=a_funda, dsout=work.b_auditfees, FeeVars= Audit_Fees Non_Audit_Fees);
...
%macro getAuditFees2(dsin=, dsout=, FeeVars= );

/* Get Funda Variables */

proc sql;
	create table funda_01 as
	select a.gvkey, b.fyear, a.datadate, b.conm, b.at, b.sale, b.tic
    from  &dsin as a left join comp.funda as b
	on a.gvkey = b.gvkey 
...
/* fill in fyear if missing */

data funda_02;
set funda_01;
if fyear = . & month(datadate) < 6 then fyear = year(datadate) - 1;
if fyear = . then fyear = year(datadate);
run;

proc sort data = funda_02 out = funda_03  nodupkey; by gvkey fyear; run;

/* Gather all potential CIKs associated with each gvkey-fyear */

proc sql;
	create table funda_04 as
	select a.gvkey, a.fyear, a.datadate, b.cik, a.conm, a.at, a.sale, a.tic,
	b.source, b.ntot, b.n10k, b.ndef
	from funda_03 as a left join wrdssec.wciklink_gvkey as b
	on a.gvkey = b.gvkey
	and b.datadate1 <= a.datadate <= b.datadate2
/* Exclude flag = 0 (No CIK Link) */ 
	and b.flag >= 1;
quit;

proc sort data = funda_04; by gvkey fyear descending ntot; run;

/* Audit Analytics */

/* Create list of company_fkey-fiscal_year_ended pairs for merge with Compustat */

/* Use upcase function to match Compustat format b/c spedis function (used later) is case sensitive */
 
proc sql;
	create table AAfees_01 as
    select distinct a.company_fkey, upcase(b.name) as Name_AA, a.fiscal_year, a.fiscal_year_ended,
/* Secondard matching variables */
	a.matchqu_balsh_assets, a.matchqu_incmst_rev_ttm,
	a.matchfy_balsh_assets, a.matchfy_incmst_rev_ttm, a.best_edgar_ticker
    from aa.auditfees as a left join aa.auditcblock as b
    on a.company_fkey = b.company_fkey;     
quit;

proc sort data = AAfees_01 nodupkey; by company_fkey fiscal_year_ended; run;

/* Fill in missing values of matching varaibles when available */

data AAfees_01;
set AAfees_01;
if MATCHfy_BALSH_ASSETS = . then MATCHfy_BALSH_ASSETS = MATCHqu_BALSH_ASSETS;
if MATCHfy_INCMST_REV_TTM = . then MATCHfy_INCMST_REV_TTM = MATCHqu_INCMST_REV_TTM;
run;

/* Possible mergers for 2nd Merger Attempt (required to match on assets & sales */

data AAfees_02;
set AAfees_01;
if MATCHfy_BALSH_ASSETS = . then delete;
if MATCHfy_INCMST_REV_TTM = . then delete;
run;

/* 1st Merge Attempt: CIK to Company_Fkey */

/* Identify possible Merge pairs */

proc sql;
	create table Merge_01 as
	select catx("|", a.gvkey, a.fyear) as gvkey_fyear, a.*, b.*
	from funda_04 as a left join AAfees_01 as b
...
/* Count number of matches for each gvkey_fyear */

proc sql;
	create table Merge_03 as
    select gvkey_fyear, gvkey, fyear, datadate, company_fkey, fiscal_year, fiscal_year_ended, count(*) as num_matches 
	from Merge_02
	group by gvkey_fyear;         
quit;

proc sort data = Merge_03; by gvkey_fyear descending Num_Matches; run;

proc sort data = Merge_03 nodupkey; by gvkey_fyear; run;

/* gvkey_fyears with only 1 cik-company_fkey match are treated as valid matches   */

data Match_01 (drop = num_matches);
set Merge_03;
if num_matches = 1;
Match = 1;
run;

/* For gvkey_fyears with multiple cik-company_fkey matches, identify proper match */

proc sql;
	create table Merge_04 (drop = num_matches) as
	select distinct a.gvkey_fyear, b.*
	from Merge_03 as a left join Merge_02 as b
	on a.gvkey_fyear = b.gvkey_fyear
	where a.num_matches > 1;
quit;

proc sort data = Merge_04; by gvkey_fyear descending ntot; run;
...
at_match = 0;
if at/1.01 <= MATCHfy_BALSH_ASSETS/1000000 <= at * 1.01 then at_match = 1;

at_diff = abs((MATCHfy_BALSH_ASSETS/1000000) - at);

sale_match = 0;
if sale/1.01 <= MATCHfy_INCMST_REV_TTM/1000000 <= sale * 1.01 then sale_match = 1;

sale_diff = abs((MATCHfy_INCMST_REV_TTM/1000000) - sale);

ticker_match = 0;
if tic = BEST_EDGAR_TICKER then ticker_match = 1;

name_match = 0;
if spedis(scan(conm,1),scan(name_aa,1)) < 10 then name_match = 1;

run;

proc sort data = Merge_05; 
by gvkey_fyear descending ticker_match descending name_match at_diff sale_diff descending ntot; run;

/* If assets and sales match, then treat as valid match */

data Match_02 (keep = gvkey_fyear gvkey fyear datadate company_fkey fiscal_year fiscal_year_ended Match);
set Merge_05;
if at_match = 1 & sale_match = 1;
Match = 1;
run;

/* Drop duplicates - tiebreaker order: 1)ticker match 2)name match 3)assets diff 4)sales diff 5)#filings */

proc sort data = Match_02 nodupkey; by gvkey_fyear; run;

/* Remove gvkey_fyears that have been matched */

proc sql;
	create table Merge_06 as
	select a.*
	from Merge_05 as a left join Match_02 as b
	on a.gvkey_fyear = b.gvkey_fyear
	where b.match ne 1;
quit;

/* Remaining duplicates: same tiebreaker order */

proc sort data = Merge_06; 
by gvkey_fyear descending ticker_match descending name_match at_diff sale_diff descending ntot; run;

proc sort data = Merge_06 out = Match_03 nodupkey; by gvkey_fyear; run;

/* Rearrange variables in Match_03 dataset */

data Match_03 (keep = gvkey_fyear gvkey fyear datadate company_fkey fiscal_year fiscal_year_ended Match);
set Match_03;
Match = 1;
run;

/* Combine Match datasets */

data Match_04;
set Match_01 Match_02 Match_03;
run;

/* 2nd Merge Attempt: Assets & Sales within 1% (for all unmatched observations) */

/* Identify remaining gvkey_fyears yet to be matched */

proc sql;
	create table Merge_07 as
	select distinct a.gvkey_fyear, a.gvkey, a.fyear, a.datadate, a.cik, a.conm, a.at, a.sale, a.tic,
	a.source, a.ntot, a.n10k, a.ndef
	from Merge_01 as a left join Match_04 as b
	on a.gvkey_fyear = b.gvkey_fyear
/* No match to company_fkey in initial merge */
	where a.company_fkey = ""
/* Remove any gvkey_fyears that were matched on a different CIK */
	and b.match ne 1
/* Remove firm years that are missing sales (required for match) */
	and a.at ne .
	and a.sale ne .;
quit;

/* Attempt to merge remaining unmatched observations based on 1% of assets and sales */

proc sql;
	create table Merge_08 as
    select a.*, b.*
	from Merge_07 as a, AAfees_02 as b
    where a.datadate - 15 <= b.FISCAL_YEAR_ENDED <= a.datadate + 15
    and a.at/1.01 <= b.MATCHfy_BALSH_ASSETS/1000000 <= a.at * 1.01
    and a.sale/1.01 <= b.MATCHfy_INCMST_REV_TTM/1000000 <= a.sale * 1.01;
quit;

data Merge_08;
set Merge_08;
gvkey_fkey = catx("|", gvkey, company_fkey);
run;

/* Count number of gvkey-company_fkey matches */
 
proc sql;
	create table Merge_09 as
    select *, count(*) as numObs 
	from Merge_08 
	group by gvkey_fkey;         
quit;

/* If gvkey and company_fkey match in more than 1 fyear, treat as valid match */

data Match_05 (keep = gvkey_fyear gvkey fyear datadate company_fkey fiscal_year fiscal_year_ended Match numObs);
set Merge_09;
if numObs > 1;
Match = 1;
run;

/* Delete more than one match in a given gvkey-fyear, keeping company_fkey with greater number of match years */

proc sort data = Match_05; by gvkey_fyear descending numObs; run;

proc sort data = Match_05 (drop = numObs) nodupkey; by gvkey_fyear; run;

/* If gvkey and company_fkey match in more only 1 fyear, then require match in either ticker or name */

data Merge_10;
set Merge_09;
if numObs = 1;
name_match = (spedis(scan(conm,1),scan(name_aa,1))<10);
ticker_match = BEST_EDGAR_TICKER eq tic;
if name_match + ticker_match > 0;
run;

/* Delete more than one match in a given gvkey-fyear, keeping company_fkey ticker match first */

proc sort data = Merge_10; by gvkey_fyear descending ticker_match; run;

proc sort data = Merge_10 nodupkey; by gvkey_fyear; run;

data Match_06 (keep = gvkey_fyear gvkey fyear datadate company_fkey fiscal_year fiscal_year_ended Match);
set Merge_10;
Match = 1;
run;

/* Combine sample of all valid matches into single dataset */

data Match_07 (drop = gvkey_fyear);
set Match_04 Match_05 Match_06;
run;

proc sort data = Match_07 nodupkey; by gvkey fyear; run;

proc sql;
	create table Output_01 as
	select a.gvkey, a.fyear, a.datadate, b.Match label 'Match to Audit Analytics?', b.company_fkey, 
	b.fiscal_year label 'Fiscal_Year (Audit Analytics)', b.Fiscal_Year_Ended label 'Fiscal_Year_Ended (Audit Analytics)'
	from Funda_02 as a left join Match_07 as b
	on a.gvkey = b.gvkey
	and a.datadate = b.datadate;
quit;
...
/* Add FeeVars specified in input */

proc sql;
	create table &dsout as
	select a.*, b.auditor_fkey, %do_over(values = &FeeVars, between = comma, phrase = b.?)
	from Output_01 as a left join aa.auditfees as b
	on a.company_fkey = b.company_fkey
    and a.fiscal_year_ended = b.fiscal_year_ended;
quit;

proc datasets library = work nolist;
   delete Funda_01 - Funda_04 Merge_01 - Merge_10 Match_01 - Match_07 Output_01 AAFees_01 - AAFees_02;
quit;
...
/****************************************************/
/* Use Macro to generate lagged variables           */
/****************************************************/

%macro lag3_yr(in=, out=, lagn=, 
id=, yr=, var=, 
sorted=0, chfyr=0, charid=0);
